...
import {
  BarChart3, Users, Unlock, Building2, LogOut, RefreshCw,
  ShieldCheck, Star, Edit2, Trash2, Check, X, ChevronDown, ChevronUp,
} from "lucide-react";
...
function AdminDashboard({ onLogout, toast }: { onLogout: () => void; toast: ReturnType<typeof useToast>["toast"] }) {
  const qc = useQueryClient();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center">
        <Card className="w-full max-w-sm text-center p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">Wrong admin secret or admin not configured.</p>
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" className="text-white hover:text-white hover:bg-slate-700" onClick={onLogout}>
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={<Unlock className="h-5 w-5 text-blue-500" />} label="Total Unlocks" value={stats?.total_unlocks ?? "—"} />
          <StatCard icon={<BarChart3 className="h-5 w-5 text-green-500" />} label="Revenue" value={stats ? formatNaira(stats.total_revenue_kobo) : "—"} />
          <StatCard icon={<Building2 className="h-5 w-5 text-purple-500" />} label="Companies" value={stats?.total_companies ?? "—"} />
          <StatCard icon={<Users className="h-5 w-5 text-orange-500" />} label="Operators" value={stats?.total_operators ?? "—"} />
        </div>
...
function UnlocksTab({ toast }: { toast: ReturnType<typeof useToast>["toast"] }) {
  ...
  if (!data?.length) return <p className="text-sm text-muted-foreground p-4">No unlocks yet.</p>;
            ...
            {data.map((u) => (
              <tr key={u.id} className="border-b hover:bg-muted/30 transition-colors">
                <td className="p-3">
                  ...
                  <p className="text-xs text-muted-foreground">{u.traveler_email}</p>
                </td>
                <td className="p-3">{u.company_name ?? "—"}</td>
                <td className="p-3 text-green-700 font-medium">{formatNaira(u.amount_paid)}</td>
                <td className="p-3 font-mono text-xs">{u.paystack_reference}</td>
                <td className="p-3">
                  {u.contact_revealed
              ...
              <tr key={c.id} className="border-b hover:bg-muted/30 transition-colors">
                ...
                <td className="p-3">
                  {editingId === c.id ? (
                    ...
                    <div className="flex items-center gap-2">
                      <code className="text-xs bg-muted px-2 py-0.5 rounded">{c.invite_code ?? "—"}</code>
                      <button
                ...
                <td className="p-3">
                  <button onClick={() => toggleVerify.mutate({ id: c.id, is_verified: !c.is_verified })}>
                    <Badge variant={c.is_verified ? "default" : "secondary"}
                      className={c.is_verified ? "bg-green-100 text-green-800 border-green-200 cursor-pointer" : "cursor-pointer"}>
                ...
                <td className="p-3">
                  <button onClick={() => toggleVerify.mutate({ id: c.id, featured: !c.featured })}>
                    <Badge variant={c.featured ? "default" : "secondary"}
                      className={c.featured ? "bg-blue-100 text-blue-800 border-blue-200 cursor-pointer" : "cursor-pointer"}>
...
function OperatorsTab({ toast }: { toast: ReturnType<typeof useToast>["toast"] }) {
  ...
  if (isLoading) return <p className="text-sm text-muted-foreground p-4">Loading…</p>;
  if (!data?.length) return <p className="text-sm text-muted-foreground p-4">No operators yet.</p>;
          ...
          <tbody>
            {data.map((op) => (
              <tr key={op.id} className="border-b hover:bg-muted/30 transition-colors">
                ...
                <td className="p-3 text-muted-foreground">{formatDate(op.created_at)}</td>
                <td className="p-3">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-destructive hover:text-destructive hover:bg-destructive/10"
                    onClick={() => { if (confirm(`Remove operator ${op.email}?`)) deleteOp.mutate(op.id); }}
                  >
