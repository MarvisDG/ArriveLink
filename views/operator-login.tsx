...
export default function OperatorLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const data = await apiPost("/operator/auth/login", { email: loginEmail, password: loginPassword });
      localStorage.setItem("operator_token", data.token);
      router.push("/operator/dashboard");
    } catch (err: unknown) {
      toast({ title: "Login failed", description: err instanceof Error ? err.message : "Please check your details", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSignup(e: React.FormEvent) {
    e.preventDefault();
    if (!signupCompanyId) {
      toast({ title: "Select your company", description: "Choose the company you represent", variant: "destructive" });
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="flex items-center justify-center gap-2 mb-8">
          ...
          <span className="font-display text-2xl font-bold text-primary">ArriveLink</span>
        </div>
          ...
          <TabsContent value="login">
            <Card>
              <CardHeader className="pb-4">
                ...
                <CardDescription>Sign in to manage your company's listings</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="signup">
            <Card>
              <CardHeader className="pb-4">
                ...
                <CardDescription>Register to manage your transport company's listing</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="space-y-2">
                    <Label>Your company</Label>
                    <Select value={signupCompanyId} onValueChange={setSignupCompanyId}>
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      required
                    />
                    <p className="text-xs text-muted-foreground">Your company's unique code — contact ArriveLink if you don't have one</p>
                  </div>
                  <Button type="submit" className="w-full" disabled={loading}>
                    {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                    Create Account
        ...
        <p className="text-center text-xs text-muted-foreground mt-6">
          ...
          <a href="/" className="text-primary hover:underline">Back to ArriveLink</a>
        </p>
      </div>
    </div>
  );
